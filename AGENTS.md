...
- Domain rule: monthly tuition is never used. Students buy a 4-session package; each attendance consumes 1 session; at 4/4 the student becomes payment-required and appears in dashboard 등록 안내.
- Admin UI lives under `src/app/(admin)` with sidebar shell; student CRM + session package logic share one store.
